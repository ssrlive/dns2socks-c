...
// on some systems a failing send command raises a signal; we want to disable
// that by passing MSG_NOSIGNAL some systems (Windows) don't define that ->
// define it to 0
#ifndef MSG_NOSIGNAL
...
struct SEntry {
    ...
    uint16_t u16aRequest[1]; // extended dynamically at malloc for "struct SEntry"
                             // (use "uint16_t" to ensure according alignment, first
                             // element contains length in big endian format)
};
...
static SOCKET g_hSockUdp = 0;                 // UDP socket
...
static uint8_t g_u8aEcsOption[8 + 16];        // EDNS Client Subnet EDNS0 option
                                              // (maximum necessary bytes reserved,
                                              // actual amount in g_iEcsOptionLen)
...
// for Windows we can create an own console window, so here we use some OS
// specific functions; also for the file access we use the WIN32 API - we could
// use the C API as we do for the other OSes but as we need to take special care
// about the line break (\r\n) anyway and WIN32 should be a little bit faster,
// we use WIN32
...
static char* GetSysError(int iErrNo)
{
    ...
    if (!FormatMessageA(flags, NULL, iErrNo, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)&szBuffer, 0, NULL)) {
...
static void OutputToLog(unsigned int uOutputSettingBits, const char* szFormatString, ...)
{
    ...
    if (uOutputSettingBits & OUTPUT_DATE_TIME) {
        ...
        GetLocalTime(&sTime);
        iLen = _snprintf_s(szBuf + iLenPrefix, ARRAYSIZE(szBuf) - iLenPrefix, _TRUNCATE, DATE_TIME_FORMAT, sTime.wYear, sTime.wMonth,
...
int create_thread_wrapper(LPTHREAD_START_ROUTINE pThreadFunction, void* pParam)
{
    DWORD uId;
...
static void OutputToLog(unsigned int uOutputSettingBits, const char* szFormatString, ...)
{
    ...
    if (uOutputSettingBits & OUTPUT_DATE_TIME) {
        ...
        iLen = snprintf(szBuf + iLenPrefix, sizeof(szBuf) - iLenPrefix, DATE_TIME_FORMAT, psTime->tm_year + 1900, psTime->tm_mon + 1,
                        psTime->tm_mday, psTime->tm_hour, psTime->tm_min, psTime->tm_sec);
        if (iLen < 0) { // error?
            return;
        }
        iLenPrefix += iLen;
...
static void OpenLogFile(const char* szFilePath, int bAppend)
{
    ...
    if (!g_pLogFile) {
        ...
        // disable buffering (no newline needed for actual output)
        setbuf(g_pLogFile, NULL);
        // output header string in log
        OutputToLog(OUTPUT_DATE_TIME, LOG_HEADER);
    }
}
...
int create_thread_wrapper(void* (*pThreadFunction)(void*), void* pParam)
{
    pthread_t hThread;
...
{
    // ignore error here
    if (psEntry->uAddrLen != sizeof(SOCKET)) { // UDP?
        // +2 because DNS on UDP doesn't include the length
        sendto(g_hSockUdp, (char*)(psEntry->u16aAnswer + 1), ntohs(*psEntry->u16aAnswer), MSG_NOSIGNAL,
               (struct sockaddr*)&psEntry->client.sAddr, psEntry->uAddrLen);
    }
    else { // TCP
...
// searches for an entry in the cache list and removes it - also closes a socket
...
static int IterateDnsMessage(uint16_t* u16aMessage, int32_t i32TimeOffset)
{
    ...
    for (; u16AmountQuestions; --u16AmountQuestions) {
        ...
        for (;;) {
            ...
            if (u8NameLen >= 0xc0) {
                // compression used? (reference to other name via offset) ignore 2nd part of offset
                ++pu8Pos;
                break;
            }
            pu8Pos += u8NameLen + 1;
    ...
    do {
        ...
        for (;;) {
            ...
            if (u8NameLen >= 0xc0) {
                // compression used? (reference to other name via offset)
    ...
    default: // found OPT pseudo-RR
    {
        ...
        for (;; pu8Pos += u16CurOptLen) {
            ...
            if (ntohs(*(uint16_t*)pu8Pos) == 8) {
                ...
                // replace existing option correct parameters for lower message creation
        ...
        *u16aDestBuf = htons((uint16_t)u32NewLen);
        ...
        memcpy(u16aDestBuf + 1, u16aRequest + 1,
               u16LenIncludingOptionsToCopy - 2); // copy part of original request without length field
        memcpy(((uint8_t*)u16aDestBuf) + u16LenIncludingOptionsToCopy, g_u8aEcsOption,
               g_iEcsOptionLen); // add ECS option
        memcpy(((uint8_t*)u16aDestBuf) + u16LenIncludingOptionsToCopy + g_iEcsOptionLen, pu8OptionsEnd,
               pu8MessageEnd - pu8OptionsEnd); // copy rest of original message
    ...
    for (;;) {
        ...
        if (caBuf[0] == 'H' || caBuf[1] == 'T' || caBuf[2] == 'T' || caBuf[3] == 'P') {
            ...
            if (*szPosStatus == ' ') {
                ...
                szPosEnd = (char*)strchr(szPosStatus, '\r');
                *szPosEnd = '\0';
...
{
    // max DNS packet length 2(length)+65535(data) bytes - using uint16_t here for alignment
    uint16_t u16aBuf[32769];
    ...
    if (g_iHttpProxyConnectLen) {
        ...
        if (g_uaUsrPwd) {
            ...
            if (((uint8_t*)u16aBuf)[17]) { // 2nd byte of answer must be 0 for "success"
                OutputToLog(OUTPUT_ALL, "The SOCKS server authentication has failed (error code %u)",
                            (unsigned int)((uint8_t*)u16aBuf)[17]);
                RemoveEntry(psEntry, hSock, g_bCacheEnabled);
                return 0;
            }
        }
        ...
        case AF_UNSPEC:                                                        // use name
            ...
            // copy name (see ParseIpAndPort)
            memcpy(((uint8_t*)u16aBuf) + 5, *(char**)&((struct sockaddr_in6*)&g_sDnsSrvAddr)->sin6_addr, iLen);
        ...
        if (((uint8_t*)u16aBuf)[0] != 5 || ((uint8_t*)u16aBuf)[1] != 0 ||
            (((uint8_t*)u16aBuf)[3] != 1 && ((uint8_t*)u16aBuf)[3] != 3 && ((uint8_t*)u16aBuf)[3] != 4)) {
        ...
        // get rest of answer all bytes after that are part of the "normal" communication
    ...
    // send DNS request via SOCKS; TCP variant starts with 2 byte length field add ECS?
    if (g_iEcsOptionLen) {
    ...
    for (psEntry = g_psFirst;; psEntry = psEntry->psNext) {
        if (!psEntry) {
            ...
            *psEntry->u16aRequest = htons((uint16_t)iLen);
            ...
            if (create_thread_wrapper(DnsThread, psEntry)) {
                ++g_uCacheCount;
...
static void OutputBindError(SOCKET hSock, struct sockaddr_storage* psAddr, int bUdp)
{
    ...
    closesocket(hSock);
    if (getnameinfo((struct sockaddr*)psAddr, GetAddrLen(psAddr), szAddr, sizeof(szAddr), szNo, sizeof(szNo),
...
THREAD_FUNCTION(TcpThread, pAddr)
{
    SOCKET hSockServer;
    SOCKET hSock;
    uint16_t* u16aBuf = NULL; // maximum possible DNS request size plus one byte - using uint16_t here for alignment;
                              // the actual size is stored in the first 2 bytes which can have a maximum value of 0xffff
    socklen_t iAddrLen;
    int iCurBufLen;
    int iLen;
    uint16_t u16ReqLen;
    ...
    u16aBuf = (uint16_t*)calloc(32769, sizeof(*u16aBuf));
#define BUFF_SIZE (32769 * sizeof(uint16_t))
    assert(u16aBuf != NULL);
    ...
    for (;;) {
        ...
        iCurBufLen = 0;
        for (;;) {
            iLen = recv(hSock, (char*)u16aBuf + iCurBufLen, BUFF_SIZE - iCurBufLen, 0);
            if (iLen <= 0) {
                szErrMsg = GetSysError(WSAGetLastError());
                closesocket(hSock);
            ...
            if (iCurBufLen >= 2 && (u16ReqLen = ntohs(*u16aBuf)) + 2 >= iCurBufLen) {
                if (u16ReqLen > 12) { // 12 bytes header plus at least one byte of data
                    // HandleDnsRequest takes care of hSock now
                    HandleDnsRequest(u16aBuf + 1, u16ReqLen, &hSock, sizeof(hSock));
                }
                else {
                    closesocket(hSock);
                }
...
static int ParseEcs(char* szIpAndAmountBits)
{
    ...
    *szPos = '\0'; // overwrite '/' for getaddrinfo below
    ...
    if (getaddrinfo(szIpAndAmountBits, NULL, &sHint, &psResult)) {
        // some getaddrinfo implementations seem to have some trouble, so try the old IPv4 variant additionally use sHint for that
...
static int ParseIpAndPort(int iFlag, const char* szParamName, const char* szPort, char* szIpAndPort, struct sockaddr_storage* psAddr)
{
    ...
    if (strchr(szIpAndPort, '.')) {
        ...
        // seems to be IPv6 format [IPv6]:port?
        if (*szIpAndPort == '[') {
            ++szIpAndPort;
            szPos = strchr(szIpAndPort, ']');
            if (szPos) {
        ...
        if (&g_sDnsSrvAddr == psAddr) {
            ...
            if (uLen < 256 && uLen) {
                ...
                if (((struct sockaddr_in6*)&g_sDnsSrvAddr)->sin6_port) {
                    // this marks usage of name
                    g_sDnsSrvAddr.ss_family = AF_UNSPEC;

                    // use sin6_addr for pointer to name (128 bit -> large enough, alignment should also be fine)
                    *(char**)&((struct sockaddr_in6*)&g_sDnsSrvAddr)->sin6_addr = szIpAndPort;

                    // use sin6_flowinfo for length
                    ((struct sockaddr_in6*)&g_sDnsSrvAddr)->sin6_flowinfo = (uint8_t)uLen;
                    return 1; // o.k.
                }
            }
        }
...
static int CreateHttpProxyConnectCommand()
{
    ...
    if (g_sDnsSrvAddr.ss_family == AF_UNSPEC) {
        ...
        int iLenDnsAddr = sprintf(g_caHttpProxyConnect + g_iHttpProxyConnectLen, "%s:%u",
                                  *(char**)&((struct sockaddr_in6*)&g_sDnsSrvAddr)->sin6_addr,
                                  ntohs(((struct sockaddr_in6*)&g_sDnsSrvAddr)->sin6_port));
        ...
        if (getnameinfo((struct sockaddr*)&g_sDnsSrvAddr, GetAddrLen(&g_sDnsSrvAddr), g_caHttpProxyConnect + g_iHttpProxyConnectLen, 128,
...
{
    OutputFatal("\nYou pressed Ctrl+C! exiting dns2socks...\n");
...
{
    // make it static so the lower array assignment causes no compiler warning
    ...
    uint16_t* u16aBuf = NULL; // max UDP packet length on Windows plus one byte - using uint16_t here for alignment
...
#ifdef _WIN32
    {
        ...
        if (iErrNo) {
            char* szErrMsg = GetSysError(iErrNo);
    ...
    for (pszCurArg = szaArgs + 1; --iArgCount; ++pszCurArg) {
        ...
        if (*szCurArg == '-' || *szCurArg == '/') {
            ...
            if (iAddrCount < sizeof(saAddresses) / sizeof(*saAddresses)) {
                if (!ParseIpAndPort(saAddresses[iAddrCount].iFlag, saAddresses[iAddrCount].szName, saAddresses[iAddrCount].szDefaultPort,
                                    szCurArg, saAddresses[iAddrCount].psAddr)) {
                    return 1;
                }
                ++iAddrCount;
                continue; // correct parameter, go to next one
        ...
        OutputFatal("\ndns2socks tunnels DNS requests via SOCKS5 and caches the "
                    "answers.\n\n\n"
                    "Usage:\n\n"
                    "dns2socks [/?] [/t] [/d] [/q] [/l[a]:FilePath] [/u:User /p:Password]\n"
                    "          [/e[f]:IP/Bits]\n"
                    "          [Socks5ServerIP[:Port]] [DNSServerIPorName[:Port]] "
                    "[ListenIP[:Port]]\n\n"
                    "/?            to view this help\n"
                    "/t            to use a HTTP proxy instead of a SOCKS server\n"
                    "              (here: Socks5ServerIP = HttpProxyIP, no support for /u "
                    "and /p)\n"
                    "/d            to disable the cache\n"
                    "/q            to suppress the text output\n"
                    "/l:FilePath   to create a new log file \"FilePath\"\n"
                    "/la:FilePath  to create a new log file or append to the existing "
                    "\"FilePath\"\n"
                    "/u:User       user name if your SOCKS server uses user/password "
                    "authentication\n"
                    "/p:Password   password if your SOCKS server uses user/password "
                    "authentication\n"
                    "/e:IP/Bits    to enable and specify EDNS client subnet in DNS "
                    "queries\n"
                    "/ef:IP/Bits   same as above but also replaces EDNS client subnet if "
                    "it exists\n\n"
                    "Default Socks5ServerIP:Port = %s:%s\n"
                    "Default DNSServerIPorName:Port = %s:%s\n"
                    "Default ListenIP:Port = %s:%s\n",
                    DEFAULT_SOCKS_SERVER, DEFAULT_SOCKS_PORT, DEFAULT_DNS_SERVER, DEFAULT_DNS_PORT, DEFAULT_LISTEN_IP, DEFAULT_DNS_PORT);
        return 1;
    }
    ...
    while (iAddrCount < sizeof(saAddresses) / sizeof(*saAddresses)) {
        struct SAddr* addr = saAddresses + iAddrCount;
        if (!ParseIpAndPort(addr->iFlag, addr->szName, addr->szDefaultPort, (char*)addr->szDefaultAddress, addr->psAddr)) {
            return 1;
        }
        ++iAddrCount;
    }
    ...
    u16aBuf = (uint16_t*)calloc(32754, sizeof(*u16aBuf));
#define BUFF_SIZE (32754 * sizeof(uint16_t))
    // convert adresses+ports to strings; use u16aBuf as string buffer
    if (getnameinfo((struct sockaddr*)&g_sSocksAddr, GetAddrLen(&g_sSocksAddr), (char*)u16aBuf, 256, (char*)u16aBuf + 256, 256,
                    NI_NUMERICHOST | NI_NUMERICSERV)) {
        // should never happen
    ...
    else if (getnameinfo((struct sockaddr*)&g_sDnsSrvAddr, GetAddrLen(&g_sDnsSrvAddr), (char*)u16aBuf + 512, 256, (char*)u16aBuf + 768, 256,
                         NI_NUMERICHOST | NI_NUMERICSERV)) {
        // should never happen
        strcpy((char*)u16aBuf + 512, "unknown address");
        strcpy((char*)u16aBuf + 768, "unknown");
    }
    if (getnameinfo((struct sockaddr*)&sAddr, GetAddrLen(&sAddr), (char*)u16aBuf + 1024, 256, (char*)u16aBuf + 1280, 256,
                    NI_NUMERICHOST | NI_NUMERICSERV)) {
        // should never happen
    ...
    OutputToLog(OUTPUT_LINE_BREAK | OUTPUT_CONSOLE,
                ...
                g_iHttpProxyConnectLen ? "HTTP proxy  " : "SOCKS server", (char*)u16aBuf, (char*)u16aBuf + 256, (char*)u16aBuf + 512,
                (char*)u16aBuf + 768, (char*)u16aBuf + 1024, (char*)u16aBuf + 1280, g_bCacheEnabled ? "enabled" : "disabled",
                szUser ? "enabled" : "disabled", (char*)u16aBuf + 1536);
    ...
    create_thread_wrapper(TcpThread, &sAddr);

    // endless loop
    for (;;) {
        // receive DNS request
        iAddrLen = sizeof(sAddr);
        iLen = recvfrom(g_hSockUdp, (char*)u16aBuf, BUFF_SIZE, 0, (struct sockaddr*)&sAddr, &iAddrLen);
        if (iLen > 12) { // 12 bytes header plus at least one byte of data
            HandleDnsRequest(u16aBuf, iLen, &sAddr, iAddrLen);
        }
        if (iLen < 0 && exiting_loop != 0) {
            // Ctrl+C handled, exiting
            break;
        }
    }
